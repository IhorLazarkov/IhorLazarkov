...
- Requests are rate-limited per browser session, not per IP (ADR-003 in `agentic-server/architecture_decision.md`). `src/service/session.ts` issues a `agentic_session` cookie via the standard `Set-Cookie` header (`HttpOnly`; `Secure; SameSite=None` in production, `SameSite=Lax` otherwise since `Secure` cookies are dropped over plain HTTP) from `GET /api/version`. `POST /api/generate` rejects with 401 (`SessionError`) if the cookie is missing — no IP fallback on that path. The session token is the key into the in-memory `RateLimiter` singleton wired through `ChatService`. Cross-origin cookies require three coordinated pieces: the cookie attributes above, `Access-Control-Allow-Credentials: true` + an origin-specific (never `*`) `Access-Control-Allow-Origin` in `src/server.ts`, and `credentials: "include"` on both `fetch` calls in `ClientToAgent.tsx`.
