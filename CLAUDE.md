...
- Requests are rate-limited per browser session, not per IP (ADR-003). `src/service/session.ts` issues an `agentic_session` cookie (`HttpOnly`; `Secure; SameSite=None` in production, `SameSite=Lax` otherwise since `Secure` cookies drop over plain HTTP) from `GET /api/version`. `GET /api/countdown` (SSE) and `POST /api/generate` both consume a rate-limit slot; `POST /api/generate` rejects with 401 (`SessionError`) if the cookie is missing — no IP fallback. Session token keys the in-memory `RateLimiter` singleton wired through `ChatService`. Cross-origin cookies need three coordinated pieces: the cookie attributes above, `Access-Control-Allow-Credentials: true` + an origin-specific (never `*`) `Access-Control-Allow-Origin` in `src/server.ts`, and `credentials: "include"` on both `fetch` calls in `ClientToAgent.tsx`.
